...
def complexMultiplication(a, b):
    ...
    res[:, :, 0] = a[:, :, 0] * b[:, :, 0] - a[:, :, 1] * b[:, :, 1]  # 对应乘积之差
    res[:, :, 1] = a[:, :, 0] * b[:, :, 1] + a[:, :, 1] * b[:, :, 0]  # 交叉乘积之和
    return res
...
class KCFTracker:
    ...
    def createGaussianPeak(self, sizey, sizex):  # 初始化
        ...
        if self._hogfeatures:
            ...
            if z.ndim == 3 and z.shape[2] == 3:
                FeaturesMap = cv2.cvtColor(z,
                                           cv2.COLOR_BGR2GRAY)  # z:(size_patch[0], size_patch[1], 3)  FeaturesMap:(
    ...
    def detect(self, z, x):
        k = self.gaussianCorrelation(x, z)  # z是_tmpl即特征的平均，x是当前帧的特征
        ...
        p[0] -= res.shape[1] / 2.
        p[1] -= res.shape[0] / 2.  # 得出偏离采样中心的位移

        return p, pv  # 返回偏离采样中心的位移和峰值

    def train(self, x, train_interp_factor):
        k = self.gaussianCorrelation(x, x)  # alphaf是频域中的相关滤波模板，有两个通道分别实部虚部
        alphaf = complexDivision(self._prob, fftd(k) + self.lambdar)  # _prob是初始化时的高斯响应图，相当于y
